...
class Collider {
    ...
    getVelocity() {
        return vec2.rotate(vec2.create(), vec2.fromValues(1, 0), vec2.fromValues(1, 0), this.getRotation());
    }

    setVelocity(x, y) {
        if(!_.isObject(x)) {
            x = vec2.fromValues(x, y);
        }

        this.setRotation(vec2.angle(x, vec2.fromValues(1, 0)));
    }
    ...
    createRect(w, h, ox = 0, oy = 0) {
        return this.addBody(util.rectPoints(w, h, ox, oy));
    }
