...
class Bullet extends Collider {
    ...
    __collideWithSceneX__() {
        ...
        if((v.x > 0) && this.__simulator__.comp('scene').collide(this, 0)) {
            this.setVelocity(vec2.set(v, -v.x, v.y));
            return;
        }

        if((v.x < 0) && this.__simulator__.comp('scene').collide(this, 2)) {
            this.setVelocity(vec2.set(v, -v.x, v.y));
            return;
        }
    }
    ...
    __collideWithSceneY__() {
        ...
        if((v.y > 0) && this.__simulator__.comp('scene').collide(this, 1)) {
            this.setVelocity(vec2.set(v, v.x, -v.y));
            return;
        }

        if((v.y < 0) && this.__simulator__.comp('scene').collide(this, 3)) {
            this.setVelocity(vec2.set(v, v.x, -v.y));
            return;
        }
    }
